...
export class EmbeddingService {
    ...
    /** True when @huggingface/transformers failed to load (e.g. sharp native binary missing). */
    ...
    async initialize() {
        ...
        this.initPromise = (async () => {
            try {
                // Generic feature-extraction pipeline for embeddings.
                // 'Xenova/all-MiniLM-L6-v2' is a lightweight embedding model.
                const { pipeline } = await import('@huggingface/transformers');
                this.extractor = await pipeline('feature-extraction', 'Xenova/all-MiniLM-L6-v2', {
                    dtype: 'q8', // Quantized weights to save memory and load faster
                });
            } catch (err: any) {
                // @huggingface/transformers requires the 'sharp' native module for image ops.
                ...
                console.warn(
                    `[EmbeddingService] @huggingface/transformers failed to load — ` +
